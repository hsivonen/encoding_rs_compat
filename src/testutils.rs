...
pub trait Testable {
    ...
    fn process_finish_err<'a>(&mut self,
                              backup: isize,
                              output: &'a Self::Output)
                              -> TestResult<'a, Self::Output>;
}
...
pub static ASCII_TEXT: &'static str = "English is a West Germanic language that was first spoken \
                                       in early medieval England and is now a global lingua \
                                       franca. It is spoken as a first language by the majority \
                                       populations of several sovereign states, including the \
                                       United Kingdom, the United States, Canada, Australia, \
                                       Ireland, New Zealand and a number of Caribbean nations; \
                                       and it is an official language of almost 60 sovereign \
                                       states. It is the third-most-common native language in the \
                                       world, after Mandarin Chinese and Spanish. It is widely \
                                       learned as a second language and is an official language \
                                       of the European Union, many Commonwealth countries and the \
...
pub static KOREAN_TEXT: &'static str = "한국어(韓國語)는 주로 한반도(韓半島)와 한민족(韓民族) 거주 지역에서 쓰이는 언어로, 대한민국에서는 \
                                        한국어, 한국말이라고 부르고, 조선민주주의인민공화국과 중국, 일본에서는 조선어(朝鮮語), 조선말이라고 \
                                        불린다. 우즈베키스탄, 러시아 등 구 소련의 고려인들 사이에서는 고려말(高麗語)로 불린다. 19세기 \
                                        중반 이후 한반도와 주변 정세의 혼란, 20세기 전반 일본 제국주의의 침략, 20세기 후반 대한민국의 \
                                        해외 이민에 의해 중국 동북 지방, 일본, 러시아 연해주와 사할린, 우즈베키스탄, 미국, 캐나다, \
                                        오스트레일리아, 필리핀, 베트남, 브라질 등 세계 곳곳에 한민족이 이주하면서 한국어가 쓰이고 있다. \
                                        한국어 쓰는 인구는 전 세계를 통틀어 약 8천250만 명으로 추산된다.";
...
pub static JAPANESE_TEXT: &'static str = "日本語（にほんご、にっぽんご）とは、主に日本国内や日本人同士の間で使われている言語である。日本は法令によって「公用語」を規定していないが、法令その他の公用文は日本語で記述され、各種法令（裁判所法第74条、会社計算規則第57条、特許法施行規則第2条など）において日本語を用いることが定められるなど事実上の公用語となっており、学校教育の「国語」でも教えられる。使用人口について正確な統計はないが、日本国内の人口、および日本国外に住む日本人や日系人、日本がかつて統治した地域の一部の住民など、約1億3千万人以上と考えられる。統計によって前後する可能性はあるが、この数は世界の母語話者数で上位10位以内に入る人数である。";
